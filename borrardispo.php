...
if (isset($_GET['id_general']) && is_numeric($_GET['id_general'])) {
    $id = intval($_GET['id_general']); // Asegúrate de que el ID sea un número entero

    // Usa una consulta preparada para evitar SQL Injection
    $stmt = $conexion->prepare("DELETE FROM general WHERE id_general = ?");
    ...
    echo '<!DOCTYPE html>
          ...
          <body>
            <script>
              Swal.fire({
                title: "Error",
                text: "No se ha proporcionado ningún ID de carrera.",
                icon: "error",
                confirmButtonText: "Aceptar"
              }).then((result) => {
                if (result.isConfirmed) {
